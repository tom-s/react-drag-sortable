...
var DragSortableList = function (_React$Component) {
  ...
  function DragSortableList(props) {
    ...
    _this.ref = 'List' + (0, _lodash.uniqueId)(); // generate unique ref
    return _this;
  }
  ...
  _createClass(DragSortableList, [{
    ...
    value: function componentWillUpdate(nextProps, nextState) {
      ...
      if (moveTransitionDuration) {
        var itemsRefs = _.union(['placeholder'], items.map(function (item) {
          return 'item-' + item.id;
        }));
    ...
    value: function componentDidUpdate(prevProps, prevState) {
      ...
      if (moveTransitionDuration) {
        ...
        if (placeholderEl && (0, _lodash.get)(prevState, 'placeholder.rank') && (0, _lodash.get)(prevState, 'placeholder.rank') !== (0, _lodash.get)(this.state, 'placeholder.rank')) {
          (function () {
            var itemsRefs = _.union(['placeholder'], _.map(items, function (item) {
              return 'item-' + item.id;
            }));
            var instructions = {
              transitions: [],
    ...
    value: function _initItems(props) {
      ...
      var newItems = items.map(function (item, i) {
        item.rank = i;
        item.id = item.id ? item.id : (0, _lodash.uniqueId)();
        return item;
      });
      this.setState({
        items: newItems
    ...
    value: function render() {
      ...
      var listItems = (0, _lodash.clone)(items, true);
      ...
      if (placeholder) {
        ...
        // Sort list
        listItems = (0, _lodash.sortBy)(listItems, function (item) {
          return item.rank;
        });
      }
    ...
    value: function _dragMove(event) {
      ...
      var state = (0, _lodash.clone)(this.state, true);
    ...
    value: function _dragEnd(event) {
      ...
      if (onSort && (0, _lodash.isFunction)(onSort)) {
        onSort(items);
      }
    }
  }, {
    ...
    value: function _moveItem() {
      ...
      var items = (0, _lodash.clone)(stateItems, true);
      ...
      items = (0, _lodash.sortBy)(items, function (item) {
        return item.rank;
      });
    ...
    value: function _movePlaceholder(e) {
      ...
      if (placeholder && placeholder.rank !== (0, _lodash.get)(this.state.placeholder, 'rank')) {
        this.setState({
